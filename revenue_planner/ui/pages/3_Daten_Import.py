...
st.set_page_config(page_title="Datenimport", page_icon="📥", layout="wide")
...
- **Datum** (z.B. `2024-01-15` oder `15.01.2024`)
- **Filialnummer** (z.B. `0120`)
- **Umsatz brutto** (Dezimalzahl, Komma oder Punkt als Trennzeichen)

Extra-Spalten werden ignoriert.
""")
...
if uploaded:
    ...
    try:
        ...
        # Auto-create missing filialen entries
        new_fil = ensure_filialen_from_ist(conn)
        if new_fil > 0:
            st.info(f"ℹ️ {new_fil} neue Filial-Einträge automatisch angelegt (bitte Bundesland prüfen → Seite Filialen).")
...
if summary.empty:
    ...
    st.metric("Filialen mit IST-Daten", len(summary))
    st.metric("Datensätze gesamt", f"{summary['tage'].sum():,}")
