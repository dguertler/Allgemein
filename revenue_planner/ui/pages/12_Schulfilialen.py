...
def detect_schulfilialen(conn_db, threshold=0.8) -> dict:
    ...
    filialen = conn_db.execute("SELECT fil_nr, bundesland FROM filialen").fetchall()
    fil_bl = {r["fil_nr"]: r["bundesland"] for r in filialen}
...
# ── Auto-detect button ─────────────────────────────────────────────────────
if st.button("Aus IST-Daten erkennen", type="secondary"):
...
# Load existing schulferien mappings
existing_sf = {
...
for f in filialen:
    ...
    for (art, bl), label in zip(vacation_cols, col_labels):
        # Only show if branch's Bundesland matches (or bl is "alle")
        fil_bl = f["bundesland"] or ""
        if bl == "alle" or fil_bl == bl:
            row[label] = existing_sf.get((f["fil_nr"], art, bl), False)
        else:
            row[label] = None  # Not applicable
    data.append(row)

df_matrix = pd.DataFrame(data)

# Separate checkbox columns
checkbox_cols = col_labels
...
if st.button("Schulfilial-Zuordnung speichern", type="primary"):
    saved = 0
