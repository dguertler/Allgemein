...
# Ramadan approximate dates (start, end) per year
RAMADAN = {
...
LOAD_YEARS = list(range(2023, 2037))  # 2023-2036
...
def _load_public_holidays_year(planjahr: int) -> list:
    """Load all German public holidays for one year (plan year uses prev year as vj)."""
...
def _feiertagstage_rows(holiday_rows: list) -> list:
    ...
    Rules:
    - Holiday on Sunday: no Feiertagstage
    - Holiday on Monday: Saturday (-2), Sunday (-1), Tuesday (+1)
    - Other days: day before (-1) and day after (+1)
    datum_vj is calculated relative to the holiday's datum_vj.
    """
    result = []
    for row in holiday_rows:
        ...
        if wd == 6:
            # Sunday: no Feiertagstage
            continue
        elif wd == 0:
            # Monday: Sat(-2), Sun(-1), Tue(+1)
            offsets = [-2, -1, 1]
        else:
            # Other: day before(-1) and day after(+1)
            offsets = [-1, 1]
...
def _sondertage_rows_year(planjahr: int, with_muttertag: bool,
    ...
    if with_fasching:
        ...
        # Full Fasching period: Weiberfastnacht (Thu, -52) through Fastnachtsdienstag (Tue, -47)
        fasching_days = [
            ("Weiberfastnacht", 52),       # Thursday
            ("Rosen-Freitag", 51),          # Friday
            ("Faschings-Samstag", 50),      # Saturday
            ("Faschings-Sonntag", 49),      # Sunday
            ("Rosenmontag", 48),            # Monday
            ("Fastnachtsdienstag", 47),     # Tuesday
        ]
    ...
    if with_ramadan and planjahr in RAMADAN:
        ...
        # Store start as reference date
        # Find previous year's Ramadan as datum_referenz
        ...
        rows.append({
            ...
            "bezeichnung": f"Ramadan (ca.) Start",
            "methode": "referenz",
            "bundesland": "alle",
        })
        rows.append({
...
def _load_all_years(years: list, with_feiertagstage: bool, with_muttertag: bool,
                    with_fasching: bool, with_ramadan: bool,
                    replace_existing: bool, conn_db) -> dict:
    """Load holidays for all given years and save to DB. Returns counts."""
    import holidays as hol_lib
    ...
    if replace_existing:
        # Delete entries for selected years
        for yr in years:
        ...
        # Also handle Ramadan entries that start in previous year (year 2031 edge case)
        for yr in years:
            if yr in RAMADAN:
                start_iso = RAMADAN[yr][0]
...
with col_opt1:
    with_feiertagstage = st.checkbox("Feiertagstage (Vor-/Nachttage) laden", value=True,
        help="Fuer jeden Feiertag werden der Vor- und Nachtag als 'feiertagstag' markiert.")
    ...
    with_fasching = st.checkbox("Fasching (Do-Di) als Sondertage", value=True,
        help="Laedt alle 6 Fasching-Tage von Weiberfastnacht bis Fastnachtsdienstag.")
with col_opt2:
    with_ramadan = st.checkbox("Ramadan (ca.) als Sondertage", value=False,
        help="Ungefaehre Ramadan-Daten (ca.) fuer 2023-2036. "
             "Nur aktivieren wenn Ramadan-Filialen betroffen sind.")
    replace_existing = st.checkbox("Bestehende Eintraege ersetzen", value=True,
        help="Loescht alle Feiertage/Sondertage mit Datum in den geladenen Jahren vor dem Import.")

st.caption(f"Jahre: {LOAD_YEARS[0]} – {LOAD_YEARS[-1]} ({len(LOAD_YEARS)} Jahre)")

if st.button("Alle Jahre laden (2023-2036)", type="primary"):
    with st.spinner("Lade Feiertage fuer alle Jahre ..."):
        try:
            ...
            st.success(
                f"Geladen: {counts['feiertage']} Feiertag-Eintraege und "
                f"{counts['sondertage']} Sondertag-Eintraege fuer {len(LOAD_YEARS)} Jahre."
            )
...
# ── Section 2: Gespeicherte Feiertage ─────────────────────────────────────
...
if existing_ft.empty:
    ...
    jahre_ft = sorted(pd.to_datetime(existing_ft["datum_plan"], errors="coerce").dt.year.dropna().unique().astype(int),
                      reverse=True)
    ...
    edited_ft = st.data_editor(
        subset_ft.drop(columns=["id"]),
        use_container_width=True,
        hide_index=True,
        key="ft_editor",
        height=350,
    )
    st.caption(f"{len(subset_ft)} Eintraege fuer {filter_jahr}" +
               (f" / {filter_art}" if filter_art != "alle" else ""))

    if st.button("Feiertage-Aenderungen speichern", key="save_ft"):
        for idx, row in edited_ft.iterrows():
            orig_id = existing_ft.loc[idx, "id"] if idx in existing_ft.index else None
            if orig_id is not None:
                conn.execute(
                    "UPDATE feiertage SET datum_plan=?, datum_vj=?, name=?, bundesland=?, art=? WHERE id=?",
                    (row["datum_plan"], row.get("datum_vj"), row["name"],
                     row["bundesland"], row.get("art", "feiertag"), int(orig_id))
                )
        conn.commit()
...
if existing_st.empty:
    ...
    jahre_st = sorted(pd.to_datetime(existing_st["datum_plan"], errors="coerce").dt.year.dropna().unique().astype(int),
                      reverse=True)
    ...
    edited_st = st.data_editor(
        subset_st.drop(columns=["id"]),
        use_container_width=True,
        hide_index=True,
        key="st_editor",
        height=300,
    )
    st.caption(f"{len(subset_st)} Eintraege fuer {filter_jahr_st}")

    if st.button("Sondertage-Aenderungen speichern", key="save_st"):
        for idx, row in edited_st.iterrows():
            orig_id = existing_st.loc[idx, "id"] if idx in existing_st.index else None
            if orig_id is not None:
                conn.execute(
                    ...
                    (row["datum_plan"], row.get("datum_referenz"), row["bezeichnung"],
                     row.get("methode", "referenz"), row["bundesland"], int(orig_id))
                )
        conn.commit()
...
st.info(
    "Schulferien werden fuer die Schulfilial-Erkennung benoetigt. "
    "Versuche, die Schulferien aus der holidays-Bibliothek zu laden."
)

# Try to load school holidays via holidays library
...
if st.button("Schulferien laden", key="load_schulferien"):
    try:
        ...
        if school_hols is None or len(school_hols) == 0:
            st.info(
                "Die holidays-Bibliothek liefert fuer dieses Bundesland/Jahr keine Schulferien. "
                "Bitte Schulferien manuell in der Tabelle unten eintragen."
            )
        else:
            # Try to group consecutive days into periods
            from itertools import groupby
            sorted_dates = sorted(school_hols.keys())
            # Group into periods by name and consecutive dates
            inserted = 0
            # Simple approach: insert each distinct vacation name as a period
            by_name: dict = {}
            ...
            for name, dates in by_name.items():
                ...
                inserted += 1
            conn.commit()
            st.success(f"{inserted} Schulferienperioden fuer {sf_bl} {sf_year} gespeichert.")
    except Exception as e:
        st.error(f"Fehler: {e}")

# Show ferien_kalender table
fk_df = pd.read_sql(
...
if fk_df.empty:
    st.info("Noch keine Schulferien hinterlegt. Daten koennen manuell eingetragen werden.")

# Manual entry / edit
st.markdown("**Schulferien-Kalender** (bearbeitbar):")
...
if st.button("Schulferien-Kalender speichern", key="save_fk"):
    # Delete all and re-insert (since we have IDs mismatched after edits)
