...
# IST aktuell vs. Budget — nur für Tage mit importiertem IST
agg["Abw. IST €"] = agg.apply(
    lambda x: round(float(x["ist_aktuell"]) - float(x["budget"]), 2)
    if not pd.isna(x["ist_aktuell"]) else None, axis=1
)
agg["Abw. IST %"] = agg.apply(
    lambda x: round(float(x["Abw. IST €"]) / float(x["budget"]) * 100, 1)
    if not pd.isna(x.get("Abw. IST €")) and float(x.get("budget", 0) or 0) != 0 else None,
    axis=1
)
...
num_cols = ["IST Basis", "+ Öffnung", "+ Verteilung", "+ Wochentag", "+ Preis",
            "+ Ferien", "+ Feiertag", "= Budget", "Δ €", "IST aktuell", "Abw. IST €"]
